...
namespace Сессия
{
    public partial class CustomAppointmentForm : DevExpress.XtraEditors.XtraForm, IDXManagerPopupMenu
    {
        ...
        public CustomAppointmentForm(SchedulerControl control, Appointment apt) : this(control, apt, false)
        {
        }
        ...
        public virtual void LoadFormData(Appointment appointment)
        {
            ...
            // Преподаватель
            SessionDBlDataSet.AppointmentsDataTable tabAppDataTable = appointmentsTableAdapter.GetData();
            var q1 = (from row in tabAppDataTable.AsEnumerable() select row.Location).Distinct().OrderBy(x => x);
            var q2 = (from row in tabAppDataTable.AsEnumerable() select row.Subject).Distinct().OrderBy(x => x);

            foreach (string sLocation in q1)
                this.tbLocation.Properties.Items.Add(sLocation.Trim());

            foreach (string sSubject in q2)
            {
                string[] words = sSubject.Split(';');
                this.tbSubject.Properties.Items.Add(words[0].Trim());
            }
        }

        private void tbLocation_CloseUp(object sender, CloseUpEventArgs e)
        {
            SessionDBlDataSet.AppointmentsDataTable tabAppDataTable = appointmentsTableAdapter.GetData();
            var q1 = (from row in tabAppDataTable.AsEnumerable() where (row.Location.Trim() == this.tbLocation.Text.Trim()) select row.Subject).Distinct().OrderBy(x => x);

            this.tbSubject.Properties.Items.Clear();
            foreach (string sSubject in q1)
            {
                string[] words = sSubject.Split(';');
                this.tbSubject.Properties.Items.Add(words[0].Trim());
            }
        ...
        private void tbSubject_CloseUp(object sender, CloseUpEventArgs e)
        {
            if (!String.IsNullOrEmpty(e.Value.ToString()))
                e.Value = e.Value + "; " + this.edtLabel.Text.Trim();
        }

        private void edtResource_Closed(object sender, ClosedEventArgs e)
        {
            SessionDBlDataSet.ResourcesDataTable tabResDataTable = resourcesTableAdapter.GetData();
            var q = (from row in tabResDataTable.AsEnumerable() where (row.Description.Trim() == this.edtResource.Text.Trim()) select row.IdSort);

            foreach (var roww in q)
                if (roww == 0)
                    if (XtraMessageBox.Show("Этим ресурсом пользоваться нельзя!", "Аудитория", MessageBoxButtons.OK, MessageBoxIcon.Exclamation) == DialogResult.OK)
                        if (this.edtResource.SelectedIndex + 1 > this.edtResource.Properties.Items.Count)
                            this.edtResource.SelectedIndex = 0;
                        else
                            this.edtResource.SelectedIndex = this.edtResource.SelectedIndex + 1;
        }

        private void edtLabel_CloseUp(object sender, CloseUpEventArgs e)
        {
            if (!String.IsNullOrEmpty(e.Value.ToString()) && !String.IsNullOrEmpty(this.tbSubject.Text.ToString()))
            {
                string[] words = this.tbSubject.Text.Split(';');
                this.tbSubject.Text = words[0] + "; " + e.Value;
            }
        }
